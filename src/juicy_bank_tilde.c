// juicy_bank~ — modal resonator bank (with dispersion fix + phase debug)
#include "m_pd.h"
#include <math.h>
#include <string.h>
#include <stdlib.h>
...
static void jb_update_density(t_juicy_bank_tilde *x){
    ...
    if (x->density_mode == DENSITY_PIVOT){
        ...
        for(int i=0;i<x->n_modes;i++){ jb_mode_t *md=&x->m[i];
        ...
        for(int j=0;j<count;j++){ int i=idxs[j]; jb_mode_t *md=&x->m[i];
...
static float jb_position_weight(float ratio_rel, float pos){
    if (pos<=0.f) return 1.f; float k = roundf(jb_clamp(ratio_rel,1.f,1e6f));
...
static void jb_update_coeffs(t_juicy_bank_tilde *x){
    for(int i=0;i<x->n_modes;i++){ float d=x->disp_target[i]-x->disp_offset[i]; x->disp_offset[i]+=0.0025f*d; }
    jb_update_density(x);
    for(int i=0;i<x->n_modes;i++){
        jb_mode_t *md=&x->m[i]; if(!md->active){ md->a1=md->a2=0.f; continue; }
        float ratio = md->ratio_now + x->disp_offset[i];
        if (i==0) ratio = md->ratio_now;
        ...
        float c=cosf(w); md->a1=2.f*r_eff*c; md->a2=-r_eff*r_eff;
    }
}
static t_int *juicy_bank_tilde_perform(t_int *w){
    ...
    jb_update_coeffs(x); jb_update_per_mode_gains(x);
            ...
            if(x->phase_rand>0.f){
                if(md->hit_cool>0) md->hit_cool--;
                    ...
                    if(!md->hit_gate && abs_target>1e-3f){
                        ...
                        y1+=k*r1; y2+=k*r2; md->hit_gate=1; md->hit_cool=(int)(x->sr*0.005f);
                        phase_hits_block++;
                    }
...
static void juicy_bank_tilde_freq(t_juicy_bank_tilde *x, t_symbol *s, int argc, t_atom *argv){ (void)s; for(int i=0;i<argc && i<x->n_modes;i++){ if(argv[i].a_type==A_FLOAT){ float v=atom_getfloat(argv+i); x->m[i].base_ratio=(v<=0.f)?0.01f:v; x->m[i].ratio_now=x->m[i].base_ratio; } } }
static void juicy_bank_tilde_decays(t_juicy_bank_tilde *x, t_symbol *s, int argc, t_atom *argv){ (void)s; for(int i=0;i<argc && i<x->n_modes;i++){ if(argv[i].a_type==A_FLOAT){ float v=atom_getfloat(argv+i); x->m[i].base_decay_ms=(v<0.f)?0.f:v; } } }
static void juicy_bank_tilde_amps(t_juicy_bank_tilde *x, t_symbol *s, int argc, t_atom *argv){ (void)s; for(int i=0;i<argc && i<x->n_modes;i++){ if(argv[i].a_type==A_FLOAT){ float v=atom_getfloat(argv+i); x->m[i].base_gain=jb_clamp(v,0.f,1.f); } } }
...
static void juicy_bank_tilde_dispersion(t_juicy_bank_tilde *x, t_floatarg f){ float v=jb_clamp(f,0.f,1.f); if(x->dispersion_last<0.f || fabsf(v-x->dispersion_last)>1e-6f){ for(int i=0;i<x->n_modes;i++){ x->m[i].disp_signature=(i==0)?0.f:jb_rng_bi(&x->rng); } x->dispersion_last=v; } x->dispersion=v; for(int i=0;i<x->n_modes;i++){ jb_mode_t *md=&x->m[i]; if(!md->active){ x->disp_target[i]=0.f; continue; } if(i==0){ x->disp_target[i]=0.f; continue; } x->disp_target[i]=jb_clamp(md->disp_signature*x->dispersion,-1.f,1.f); } }
static void juicy_bank_tilde_seed(t_juicy_bank_tilde *x, t_floatarg f){ jb_rng_seed(&x->rng, (unsigned int)((int)f*2654435761u)); for(int i=0;i<x->n_modes;i++){ x->m[i].disp_signature=(i==0)?0.f:jb_rng_bi(&x->rng); } x->dispersion_last=x->dispersion; for(int i=0;i<x->n_modes;i++){ if(!x->m[i].active || i==0){ x->disp_target[i]=0.f; continue; } x->disp_target[i]=jb_clamp(x->m[i].disp_signature*x->dispersion,-1.f,1.f); } }
static void juicy_bank_tilde_dispersion_reroll(t_juicy_bank_tilde *x){ for(int i=0;i<x->n_modes;i++){ x->m[i].disp_signature=(i==0)?0.f:jb_rng_bi(&x->rng); } float d=x->dispersion; x->dispersion_last=-1.f; juicy_bank_tilde_dispersion(x,d); }
...
static void juicy_bank_tilde_reset(t_juicy_bank_tilde *x){ for(int i=0;i<x->n_modes;i++){ x->m[i].active=0; x->disp_offset[i]=x->disp_target[i]=0.f; x->m[i].y1=x->m[i].y2=x->m[i].drive=x->m[i].env=0.f; x->m[i].hit_gate=0; x->m[i].hit_cool=0; x->m[i].y_pre_last=0.f; } x->n_modes=1; x->sel_index=0; x->sel_index_f=1.f; jb_mode_t *md=&x->m[0]; md->active=1; md->base_ratio=1.f; md->ratio_now=1.f; md->base_gain=0.5f; md->gain_now=0.5f; md->base_decay_ms=500.f; md->decay_ms_now=md->base_decay_ms*(1.f-x->damping); md->curve_amt=0.f; md->attack_ms=0.f; md->pan=0.f; }
