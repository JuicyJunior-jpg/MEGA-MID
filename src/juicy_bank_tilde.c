...
// juicy_bank~ — modal resonator bank (V5.0 with per-voice outputs)
// Adds 8 per-voice signal OUTLETS in this exact order:
//   v1_L, v1_R, v2_L, v2_R, v3_L, v3_R, v4_L, v4_R
// Keeps the legacy stereo sum outs (outL, outR) for backward compatibility.
//
// INLETS (Signal, left→right):
//   inL, inR, v1_L, v1_R, v2_L, v2_R, v3_L, v3_R, v4_L, v4_R
//
// OUTLETS (Signal, left→right):
//   outL, outR, v1_L, v1_R, v2_L, v2_R, v3_L, v3_R, v4_L, v4_R
//
// Build (macOS):
...
typedef struct _juicy_bank_tilde {
    ...
    // OUTS
    t_outlet *outL, *outR;                 // legacy stereo sum
    t_outlet *out_vL[JB_MAX_VOICES];       // new per-voice L
    t_outlet *out_vR[JB_MAX_VOICES];       // new per-voice R
...
static float jb_bright_gain(float ratio_rel, float b){
    float t=(jb_clamp(b,0.f,1.f)-0.5f)*2.f; float p=0.6f*t; float rr=ratio_rel<1.f?1.f:ratio_rel;
    return powf(rr, p);
}
static float jb_position_weight(float ratio_rel, float pos){
    if (pos<=0.f) return 1.f;
    float k = roundf(ratio_rel<1.f?1.f:ratio_rel);
...
static inline float jb_curve_shape_gain(float u, float curve){
    ...
    else            { float t =  curve; gamma = 1.f + t*(3.0f - 1.f);  }
    float phi = powf(jb_clamp(u,0.f,1.f), gamma);
    float delta = phi - u;
    float g = powf(10.f, -3.f * delta);
    if (curve < 0.f && g < 0.001f) g = 0.001f; // floor to avoid denorm/quant hiss
    return g;
}
...
// ---------- sympathetic update ----------
static int jb_is_near_integer_local(float x, float eps){ float n=roundf(x); return fabsf(x-n)<=eps; }
static void jb_update_crossring(t_juicy_bank_tilde *x, int self_idx){
    ...
    for(int m=0;m<JB_MAX_MODES;m++){ x->v[self_idx].cr_gain_mul[m]=1.f; x->v[self_idx].cr_decay_mul[m]=1.f; }
    ...
    for(int u=0; u<x->max_voices; ++u){
        ...
        for(int m=0;m<x->n_modes;m++){
            ...
            if (jb_is_near_integer_local(rel, eps)){
...
static void jb_voice_reset_states(const t_juicy_bank_tilde *x, jb_voice_t *v, jb_rng_t *rng){
    ...
    for(int i=0;i<x->n_modes;i++){
        ...
        v->cr_gain_mul[i]=v->cr_decay_mul[i]=1.f;
        (void)rng;
    }
}
...
static t_int *juicy_bank_tilde_perform(t_int *w){
    ...
    t_sample *v1L_in=(t_sample *)(w[4]);
    t_sample *v1R_in=(t_sample *)(w[5]);
    t_sample *v2L_in=(t_sample *)(w[6]);
    t_sample *v2R_in=(t_sample *)(w[7]);
    t_sample *v3L_in=(t_sample *)(w[8]);
    t_sample *v3R_in=(t_sample *)(w[9]);
    t_sample *v4L_in=(t_sample *)(w[10]);
    t_sample *v4R_in=(t_sample *)(w[11]);
    // outputs (10): mix L/R then 8 per-voice outs
    ...
    for(int i=0;i<n;i++){
        outL[i]=outR[i]=0.f;
        ov1L[i]=ov1R[i]=ov2L[i]=ov2R[i]=ov3L[i]=ov3R[i]=ov4L[i]=ov4R[i]=0.f;
    }
    ...
    t_sample *vinL[JB_MAX_VOICES] = { v1L_in, v2L_in, v3L_in, v4L_in };
    t_sample *vinR[JB_MAX_VOICES] = { v1R_in, v2R_in, v3R_in, v4R_in };
    t_sample *voutL[JB_MAX_VOICES] = { ov1L, ov2L, ov3L, ov4L };
    t_sample *voutR[JB_MAX_VOICES] = { ov1R, ov2R, ov3R, ov4R };
    ...
    for(int vix=0; vix<x->max_voices; ++vix){
        ...
        for(int m=0;m<x->n_modes;m++){
            ...
            for(int i=0;i<n;i++){
                ...
                if(absL>1e-3f){
                    ...
                    if(!md->hit_gateL){
                        if(x->phase_rand>0.f){
                            ...
                            y1L+=k*jb_rng_bi(&x->rng); y2L+=k*jb_rng_bi(&x->rng);
                            if (bw_amt>0.f){ y1bL+=k*jb_rng_bi(&x->rng); y2bL+=k*jb_rng_bi(&x->rng); }
                        }
                ...
                if(absR>1e-3f){
                    ...
                    if(!md->hit_gateR){
                        if(x->phase_rand>0.f){
                            ...
                            y1R+=k*jb_rng_bi(&x->rng); y2R+=k*jb_rng_bi(&x->rng);
                            if (bw_amt>0.f){ y1bR+=k*jb_rng_bi(&x->rng); y2bR+=k*jb_rng_bi(&x->rng); }
                        }
                ...
                // per-mode pan and accumulation
                float p = jb_clamp(x->base[m].pan, -1.f, 1.f);
                float wL = sqrtf(0.5f*(1.f - p));
                float wR = sqrtf(0.5f*(1.f + p));
                float oL = y_totalL * wL;
                float oR = y_totalR * wR;

                // sum to mix
                outL[i] += oL;
                outR[i] += oR;
                // sum to per-voice outs
                voutL[vix][i] += oL;
                voutR[vix][i] += oR;

                // envelopes
                float ayL=fabsf(oL); envL = envL + 0.0015f*(ayL - envL); md->y_pre_lastL = oL;
                float ayR=fabsf(oR); envR = envR + 0.0015f*(ayR - envR); md->y_pre_lastR = oR;
            }
    ...
    // DC high-pass on mix only (per-voice outs are "raw" by design; WG will do its own DC hygiene)
    ...
    return (w + 23); // x + 22 args (10 in, 10 out, n) -> next
}

// ---------- base setters & messages (subset to keep file concise here) ----------
...
static void juicy_bank_tilde_dsp(t_juicy_bank_tilde *x, t_signal **sp){
    ...
    // sp layout: [10 in] + [10 out]
    t_int argv[1 + 22]; // x + 22 args (10 in, 10 out, n)
    int a=0;
    argv[a++] = (t_int)x;
    // 10 inputs
    for(int k=0;k<10;k++) argv[a++] = (t_int)(sp[k]->s_vec);
    // 10 outputs
    for(int k=10;k<20;k++) argv[a++] = (t_int)(sp[k]->s_vec);
    // n
    argv[a++] = (int)(sp[0]->s_n);
...
static void juicy_bank_tilde_free(t_juicy_bank_tilde *x){
    ...
    for(int i=0;i<JB_MAX_VOICES;i++){ if(x->out_vL[i]) outlet_free(x->out_vL[i]); if(x->out_vR[i]) outlet_free(x->out_vR[i]); }
}
...
static void *juicy_bank_tilde_new(void){
    ...
    // OUTS: first stereo mix, then 8 per-voice outs in order v1_L..v4_R
    x->outL = outlet_new(&x->x_obj, &s_signal);
    x->outR = outlet_new(&x->x_obj, &s_signal);
    for(int i=0;i<JB_MAX_VOICES;i++){
        x->out_vL[i] = outlet_new(&x->x_obj, &s_signal);
        x->out_vR[i] = outlet_new(&x->x_obj, &s_signal);
    }
    return (void *)x;
}
...
void juicy_bank_tilde_setup(void){
    ...
    // a few core setters (add back others from your full file as needed)
