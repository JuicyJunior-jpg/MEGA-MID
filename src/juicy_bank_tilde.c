// juicy_bank~ — modal resonator bank (clean build)
// Features:
//  • 64 modes default, INIT/init message restores defaults
//  • spacing inlet repurposed → density pivot bias (0..1) [0=fundamental pivot, 1=neighbor mapping]
//  • dispersion now bipolar [-1,1]:
//      <0 : harmonic-quantize morph to nearest integer ratio
//      >0 : randomized inharmonicity using per-mode signatures (stiffen augments amount)
//  • per-mode resonance normalization so lows aren’t inherently louder
//
// Build (Linux):
...
typedef enum { DENSITY_PIVOT=0 } jb_density_mode;

typedef struct {
    float base_ratio, base_decay_ms, base_gain;
    float attack_ms, curve_amt, pan;
    int   active;
    int   keytrack; // 1=track base f0, 0=absolute Hz
...
typedef struct {
    ...
    // per-hit randoms
    float md_hit_offsetL, md_hit_offsetR;
    float bw_hit_ratioL, bw_hit_ratioR;
...
typedef struct _juicy_bank_tilde {
    ...
    float spacing; // repurposed: density pivot bias 0..1
...
static void jb_project_behavior_into_voice(t_juicy_bank_tilde *x, jb_voice_t *v){
    ...
    // positive dispersion only uses signatures
    float pos_disp = (x->dispersion > 0.f) ? x->dispersion : 0.f;
    float total_disp = jb_clamp(pos_disp + v->stiffen_add, 0.f, 1.f);
    for(int i=0;i<x->n_modes;i++){
...
// ---------- sympathetic coupling (simple version) ----------
static void jb_update_crossring(t_juicy_bank_tilde *x, int self_idx){
    const float eps = 0.02f + 0.02f * x->crossring_amt;
    const float gmul = 1.f + 0.10f * x->crossring_amt;
    const float dmul = 1.f + 0.15f * x->crossring_amt;
...
static void jb_update_voice_coeffs(t_juicy_bank_tilde *x, jb_voice_t *v){
    ...
    for(int i=0;i<x->n_modes;i++){
        ...
        // Negative dispersion morph (quantize toward nearest integer)
        if (x->dispersion < 0.f){
            float a = jb_clamp(-x->dispersion, 0.f, 1.f);
        ...
        // frequency-normalized drive factors
        ...
        for(int m=0;m<x->n_modes;m++){
            ...
            for(int i=0;i<n;i++){
                ...
                if (camt > 0.f){
                    ...
                    float xL = y_totalL * drive; float biasL = asym * (xL >= 0.f ? (xL*xL) : -(xL*xL)); xL += biasL;
                    float xR = y_totalR * drive; float biasR = asym * (xR >= 0.f ? (xR*xR) : -(xR*xR)); xR += biasR;
                    float k = 0.6f;
...
static void juicy_bank_tilde_modes(t_juicy_bank_tilde *x, t_floatarg nf){
    ...
    for(int i=0;i<x->n_modes;i++){ if(x->base[i].base_ratio<=0.f) x->base[i].base_ratio=(float)(i+1); }
}
...
static void juicy_bank_tilde_ratio_i(t_juicy_bank_tilde *x, t_floatarg r){
    ...
    if (x->base[i].keytrack){ x->base[i].base_ratio = (r<=0.f)?0.01f:r; }
    else { float ui = r; if (ui < 0.f) ui = 0.f; if (ui > 10.f) ui = 10.f; x->base[i].base_ratio = 100.f * ui; }
}
...
static void juicy_bank_tilde_spacing(t_juicy_bank_tilde *x, t_floatarg f){ x->spacing=jb_clamp(f,0.f,1.f); } // pivot bias
...
static void juicy_bank_tilde_seed(t_juicy_bank_tilde *x, t_floatarg f){
    ...
    x->dispersion_last=(x->dispersion>0.f)?x->dispersion:0.f;
}
...
static void jb_apply_default_saw(t_juicy_bank_tilde *x){
    ...
    for(int i=0;i<JB_MAX_MODES;i++){
        ...
        x->base[i].base_decay_ms = 1000.f;
        x->base[i].base_gain = 1.0f / (float)(i+1); // true saw 1/n
...
static void *juicy_bank_tilde_new(void){
    ...
    // body: damping, brightness, position, density, dispersion, spacing(pivot bias), anisotropy, contact
    ...
    x->in_spacing    = inlet_new(&x->x_obj, &x->x_obj.ob_pd, &s_float, gensym("spacing"));
    ...
    x->in_decya      = inlet_new(&x->x_obj, &x->x_obj.ob_pd, &s_float, gensym("decya"));
...
static void juicy_bank_tilde_INIT(t_juicy_bank_tilde *x){
    ...
    post("juicy_bank~: INIT complete (64 modes, saw-like 1/n gains).");
}
...
void juicy_bank_tilde_setup(void){
    ...
    class_addmethod(juicy_bank_tilde_class, (t_method)juicy_bank_tilde_spacing, gensym("spacing"), A_DEFFLOAT, 0);
    ...
    class_addmethod(juicy_bank_tilde_class, (t_method)juicy_bank_tilde_decay_i, gensym("decya"), A_DEFFLOAT, 0);
