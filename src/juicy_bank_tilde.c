...
static float jb_hw_quantize_param_value(jb_hw_param_t pid, float value){
    switch(pid){
        case JB_HW_PARAM_DENSITY: {
            /* Density arrives here in its public display domain (0..6, with 1 = normal).
               Quantize directly in that domain, then let the apply path convert to legacy UI. */
            float d = jb_clamp(value, 0.f, 6.f);
            if(d >= 1.f){
