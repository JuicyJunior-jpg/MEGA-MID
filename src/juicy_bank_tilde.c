...
static t_int *juicy_bank_tilde_perform(t_int *w){
    ...
    for (int k=1; k<=N; ++k){
        ...
        double amp = (double)m->gain * (double)x->mix_scale * (double)m->amp_w; // aniso already in amp_w
